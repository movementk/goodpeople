<!-- Header -->
<header id="header">
    <h1 class="logo">
        <a href="/"><img src="/assets/images/logo.jpg" alt="logo"></a>
    </h1>
    <nav id="gnb" class="d-none d-xl-block">
        <ul class="gnb-menu">
            <li>
                <a href="#">굿피플</a>
                <div class="introduce">
                    <div class="intro">
                        <h2>굿피플</h2>
                        <p>
                            굿피플은 지구촌 희망건설에 앞장서는 <br>
                            국제구호개발 NGO 입니다.
                        </p>
                        <figure>
                            <img src="/assets/images/main/img_introduce.png" alt="">
                        </figure>
                    </div>
                    <div class="page-map">
                        <ul class="page-list">
                            <li>
                                <h3>소개</h3>
                                <ul>
                                    <li><a href="/introduce/vision.php">비전과 가치<span></span></a></li>
                                    <li><a href="/introduce/history.php">걸어온 길<span></span></a></li>
                                    <li><a href="/introduce/greetings.php">인사말<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>함께하는 사람들</h3>
                                <ul>
                                    <li><a href="/introduce/ambassadors.php">나눔대사<span></span></a></li>
                                    <li><a href="/introduce/directors.php">조직도<span></span></a></li>
                                    <li><a href="/introduce/directorate.php">이사회 및 위원회<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>투명경영</h3>
                                <ul>
                                    <li><a href="/introduce/corporation.php">사단법인<span></span></a></li>
                                    <li><a href="/introduce/welfarecorp.php">복지법인<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>지부안내</h3>
                                <ul>
                                    <li><a href="/introduce/international_camps.php">해외지부<span></span></a></li>
                                    <li><a href="#">국내지부<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>찾아오시는 길</h3>
                                <ul>
                                    <li><a href="/introduce/directions.php">찾아오시는 길<span></span></a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
                </div>
            </li>
            <li>
                <a href="#">사업소개</a>
                <div class="business">
                    <div class="intro">
                        <h2>사업소개</h2>
                        <p>
                            굿피플은 지구촌 희망건설에 앞장서는 <br>
                            국제구호개발 NGO 입니다.
                        </p>
                        <figure>
                            <img src="/assets/images/main/img_introduce.png" alt="">
                        </figure>
                    </div>
                    <div class="page-map">
                        <ul class="page-list">
                            <li>
                                <h3>해외사업</h3>
                                <ul>
                                    <li><a href="#">아동후원사업<span></span></a></li>
                                    <li><a href="#">보건의료사업<span></span></a></li>
                                    <li><a href="#">교육지원사업<span></span></a></li>
                                    <li><a href="#">지역개발사업<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>국내사업</h3>
                                <ul>
                                    <li><a href="#">국내아동후원<span></span></a></li>
                                    <li><a href="#">아동지원사업<span></span></a></li>
                                    <li><a href="#">위기가정지원사업<span></span></a></li>
                                    <li><a href="#">통합지원사업 문화<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>사랑의의료봉사</h3>
                                <ul>
                                    <li><a href="/business/volunteer_introduce.php">소개<span></span></a></li>
                                    <li><a href="/business/volunteer_schedule.php">스케줄<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>긴급구호</h3>
                                <ul>
                                    <li><a href="/business/emergency_relief.php">긴급구호<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>사회복지사업</h3>
                                <ul>
                                    <li><a href="/business/welfare_work.php">사회복지사업<span></span></a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
                </div>
            </li>
            <li>
                <a href="#">캠페인</a>
                <div class="campaign">
                    <div class="intro">
                        <h2>캠페인</h2>
                        <p>
                            굿피플은 지구촌 희망건설에 앞장서는 <br>
                            국제구호개발 NGO 입니다.
                        </p>
                        <figure>
                            <img src="/assets/images/main/img_introduce.png" alt="">
                        </figure>
                    </div>
                    <div class="page-map">
                        <ul class="page-list">
                            <li>
                                <h3>캠페인</h3>
                                <ul>
                                    <li><a href="#">캠페인<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>캠페인 후기</h3>
                                <ul>
                                    <li><a href="/campaign/campaign_review.php">캠페인 후기<span></span></a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
                </div>
            </li>
            <li>
                <a href="#">소식</a>
                <div class="news">
                    <div class="intro">
                        <h2>소식</h2>
                        <p>
                            굿피플은 지구촌 희망건설에 앞장서는 <br>
                            국제구호개발 NGO 입니다.
                        </p>
                        <figure>
                            <img src="/assets/images/main/img_introduce.png" alt="">
                        </figure>
                    </div>
                    <div class="page-map">
                        <ul class="page-list">
                            <li>
                                <h3>굿피플 소식</h3>
                                <ul>
                                    <li><a href="/community/news_list.php">소식<span></span></a></li>
                                    <li><a href="/community/notice_list.php">공지사항<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>사업소식</h3>
                                <ul>
                                    <li><a href="/community/business_list.php">해외/국내사업이야기<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>언론보도</h3>
                                <ul>
                                    <li><a href="/community/media_report.php">언론보도<span></span></a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
                </div>
            </li>
            <li>
                <a href="#">후원하기</a>
                <div class="sponsorship">
                    <div class="intro">
                        <h2>후원하기</h2>
                        <p>
                            굿피플은 지구촌 희망건설에 앞장서는 <br>
                            국제구호개발 NGO 입니다.
                        </p>
                        <figure>
                            <img src="/assets/images/main/img_introduce.png" alt="">
                        </figure>
                    </div>
                    <div class="page-map">
                        <ul class="page-list">
                            <li>
                                <h3>정기후원</h3>
                                <ul>
                                    <li><a href="/sponsor/regular_sponsor.php">정기후원<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>일시후원</h3>
                                <ul>
                                    <li><a href="/mypage/my_support/children_list_type1.php">일시후원<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>아너스클럽</h3>
                                <ul>
                                    <li><a href="/sponsor/honors_club.php">아너스클럽<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>기업후원</h3>
                                <ul>
                                    <li><a href="/sponsor/share_partner.php">기업후원<span></span></a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
                </div>
            </li>
            <li>
                <a href="#">나의후원</a>
                <div class="sponsorship">
                    <div class="intro">
                        <h2>나의후원</h2>
                        <p>
                            굿피플은 지구촌 희망건설에 앞장서는 <br>
                            국제구호개발 NGO 입니다.
                        </p>
                        <figure>
                            <img src="/assets/images/main/img_introduce.png" alt="">
                        </figure>
                    </div>
                    <div class="page-map">
                        <ul class="page-list">
                            <li>
                                <h3>나의 후원아동</h3>
                                <ul>
                                    <li><a href="/mypage/my_support/children_list_type1.php">나의 후원아동<span></span></a></li>
                                    <li><a href="/mypage/my_support/letter_list.php">편지쓰기<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>회원정보변경</h3>
                                <ul>
                                    <li><a href="#">회원정보변경<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>후원 내역</h3>
                                <ul>
                                    <li><a href="#">후원 내역<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>기부금영수증</h3>
                                <ul>
                                    <li><a href="#">기부금영수증<span></span></a></li>
                                </ul>
                            </li>
                            <li>
                                <h3>문의</h3>
                                <ul>
                                    <li><a href="/mypage/inquiry/inquiry_list.php">문의<span></span></a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
                </div>
            </li>
        </ul>
        <div class="quick-menu">
            <ul class="member">
                <li><a href="/member/login.php">로그인</a></li>
                <li><a href="#">회원가입</a></li>
                <li><a href="/member/annual_report.php">자료실</a></li>
            </ul>
            <div class="language">
                <div class="dropdown">
                    <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton-1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        KOR
                    </button>
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton-1">
                        <a class="dropdown-item" href="#" target="_blank">ENG</a>
                    </div>
                </div>
            </div>
            <div class="sponsorship">
                <div class="dropdown">
                    <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton-2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        후원하기
                    </button>
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton-2">
                        <a href="/sponsor/regular_sponsor.php" class="dropdown-item">정기후원</a>
                        <a href="/sponsor/temporary_sponsor.php" class="dropdown-item">일시후원</a>
                        <a href="/sponsor/honors_club.php" class="dropdown-item">아너스클럽</a>
                        <a href="/sponsor/share_partner.php" class="dropdown-item">기업후원</a>
                    </div>
                </div>
            </div>
        </div>
    </nav>
</header>